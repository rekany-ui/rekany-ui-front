...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Préparer les données pour Laravel (mapping des champs)
      const sujetLabel = subjects.find(s => s.id === selectedSubject)?.label || "Autre";

      const data = {
        nom: formData.name,        // name → nom
        email: formData.email,
        telephone: formData.phone, // phone → telephone
        entreprise: formData.company, // company → entreprise
        sujet: sujetLabel,
                ...
                <div className="relative">
                  ...
                  <form onSubmit={handleSubmit} className="mt-8 space-y-6" noValidate>
                    ...
                    <div>
                      ...
                      />
                      <div className="mt-1 text-right text-xs text-rekany-gray/40">
                    ...
                    <div className="flex flex-col-reverse items-center gap-4 pt-2 sm:flex-row sm:justify-between">
                      ...
                      <motion.button
                          ...
                          whileTap={{ scale: 0.98 }}
                          className="group inline-flex w-full items-center justify-center gap-2 rounded-xl bg-rekany-dark px-7 py-3.5 font-poppins text-sm font-semibold text-white shadow-md transition-all hover:bg-rekany-dark/90 disabled:opacity-50 disabled:cursor-not-allowed sm:w-auto"
                      >
                        {sent ? (
                            <>
