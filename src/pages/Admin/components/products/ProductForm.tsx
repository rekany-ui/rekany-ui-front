...
interface ProductFormProps {
  ...
  formValues: Partial<Product>;
  onFormChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void;
  onFileChange?: (e: React.ChangeEvent<HTMLInputElement>) => void; // Nouveau pour le fichier
  mode: FormMode;
  editingId: string | null;
  imagePreview?: string | null; // Pour afficher un aperçu
}
              ...
              return (
                <div key={field.name}>
                  ...
                  <input
                    ...
                    required={field.required && !editingId} // Requis seulement pour la création
                    className="w-full rounded-xl border border-gray-200 bg-rekany-beige/50 px-4 py-2.5 text-sm focus:border-transparent focus:ring-2 focus:ring-rekany-light transition-shadow file:mr-4 file:rounded-lg file:border-0 file:bg-rekany-light/20 file:px-4 file:py-2 file:text-sm file:font-medium file:text-rekany-dark hover:file:bg-rekany-light/30"
                  />
                  {editingId && (
